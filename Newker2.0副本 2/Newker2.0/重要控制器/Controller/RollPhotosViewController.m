...
- (void)createView
{
    ...
    self.imgsumLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-35, [UIScreen mainScreen].bounds.size.width * 43/40 + 5, 35, 30)];
    _setnameLabel.textAlignment = NSTextAlignmentLeft;
    self.imgsumLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-50, [UIScreen mainScreen].bounds.size.height-170, 50, 30)];
    _imgsumLabel.textColor = [UIColor whiteColor];
    _imgsumLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    
    //内容
    self.descLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, [UIScreen mainScreen].bounds.size.width * 43/40 + 40, [UIScreen mainScreen].bounds.size.width-20, [UIScreen mainScreen].bounds.size.width * 17/40 - 40)];
    _imgsumLabel.textAlignment = NSTextAlignmentRight;
    self.descLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, [UIScreen mainScreen].bounds.size.height-150, [UIScreen mainScreen].bounds.size.width-40, 100)];
