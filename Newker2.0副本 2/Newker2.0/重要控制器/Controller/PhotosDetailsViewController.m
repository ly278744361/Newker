...
- (void)createView
{
    ...
    self.ptimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(110, 40, 130, 20)];
    _ptimeLabel.font = [UIFont boldSystemFontOfSize:12.f];
    _ptimeLabel.alpha = 0.5;
    
    self.contactLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 1200,[UIScreen mainScreen].bounds.size.width-40, 570)];
    
    
    
    
    NSLog(@"--------------%f", self.contactLabel.frame.origin.y);
...
- (void)setData:(Details *)details
{
    ...
    for (int i = 0; i<[_details.img count]; i++) {
        ...
        UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(20, i*290+80,[UIScreen mainScreen].bounds.size.width-40 , 280)];
    ...
    _contactLabel.frame = CGRectMake(_contactLabel.frame.origin.x,[details.img count]*280+100, _contactLabel.frame.size.width, size.height);
    //ScrollView的自适应高度
